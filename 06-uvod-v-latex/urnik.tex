...
\begin{enumerate}
   ...
   \item Če nekaj časa kljub spreminjanju operacij nad urnikom vedno znova obstojimo na isti
   \item rešitvi, potem to najverjetneje pomeni, da smo prišli do lokalnega ekstrema. Drugi
   \item ustavitveni pogoj je zadovoljivo velika vrednost kriterijske funkcije.

   \item Naključno izberemo okolico trenutne rešitve. Z okolico izberemo tudi operacijo,
   \item ki jo izvajamo na tem koraku (npr. zamenjava dveh srečanj).

   \item Če v tem koraku izberemo tisto zamenjavo srečanj, ki izmed vseh možnih najbolj
   \item poveča vrednost kriterijske funkcije, potem postopamo enako, kot požrešna metoda. Če
   \item želimo nadgraditi postopek lokalnega vzpenjanja, ne potrebujemo nujno najboljše možne
   \item zamenjave, ampak prvo, ki poveča vrednost kriterijske funkcije. Izjemoma dovolimo tudi
   \item tisto zamenjavo, ki vrednost kriterijske funkcije nekoliko zmanjša, kar se na prvi
   \item pogled zdi nesmiselno, nas pa dolgoročno pripelje do boljših rešitev.

   \item Izbrano zamenjavo dveh srečanj izvedemo na trenutnem urniku. Vidimo, da en korak
   \item lokalnega vzpenjanja res predstavlja eno spremembo nad urnikom, v konkretnem primeru
   \item smo izvedli (eno) zamenjavo dveh srečanj.
\end{enumerate}
